...
### 시스템 구성도
<img src="./기타자료/system-architecture.jpg" alt="시스템 구성도" width="450" />

* **클라이언트 Client**
  + 사용자 인증 및 얼굴 등록 관리
    - 자신의 얼굴 사진 관리 (1 ~ 5개)<sup> [1]</sup>
    - 사전에 인증된 아이디를 입력해 **촬영 대상**<sup> 인가 인원</sup> 선택

  + 모자이크 요청
    - 실시간 미디어 촬영 혹은 이미 저장된 미디어 선택
    - 모자이크 처리 결과물 확인 및 저장
...
* **서버 Server**
  ...
  + Flask 서버
    - **촬영 대상 외의 인원**<sup> 비인가 인원 </sup>에 모자이크 처리<sup> [3]</sup>

> [1] 얼굴 사진은 모자이크 처리 시 인가 얼굴 객체로 사용된다.<br/>
> [2] 회원가입, 로그인, 얼굴 사진 관리, 인가 얼굴 객체 선택에 대한 요청을 처리한다.<br/>
> [3] 실시간 모자이크 처리는 클라이언트로부터 실시간으로 촬영되는 카메라 화면을 수신한 후,
> 각 프레임마다 객체 분석과 모자이크 생성 단계를 반복하여 수행된다.
